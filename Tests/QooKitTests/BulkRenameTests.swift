...
@Suite struct BulkRenameTests {
    ...
    @Test func numbersItemsInTheGivenOrder() {
        ...
        #expect(changes[0].newName == "作品 1.cbz")
        #expect(changes[1].newName == "作品 2.cbz")
    }
    ...
    @Test func honoursTheStartNumber() {
        ...
        #expect(changes[0].newName == "作品 10.cbz")
        #expect(changes[1].newName == "作品 11.cbz")
    }

    @Test func counterStyleIsZeroPadded() {
        let changes = BulkRename.plan(
            names: ["a.cbz"],
            mode: .format(style: .nameAndCounter, customText: "作品", placement: .after, startNumber: 1)
        )
        #expect(changes[0].newName == "作品 00001.cbz")
    }
    ...
    @Test func numberCanGoBeforeTheName() {
        ...
        #expect(changes[0].newName == "3 作品.cbz")
    }

    /// カスタム文字列が空なら、元の名前（拡張子を除く）を使う。
    @Test func emptyCustomTextKeepsTheOriginalStem() {
        let changes = BulkRename.plan(
            names: ["作品A.cbz"],
            mode: .format(style: .nameAndIndex, customText: "", placement: .after, startNumber: 1)
        )
        #expect(changes[0].newName == "作品A 1.cbz")
    }
    ...
    @Test func dateStyleUsesAFileSafeFormat() {
        ...
        #expect(changes[0].newName.hasPrefix("作品 2026-08-15"))
    }
