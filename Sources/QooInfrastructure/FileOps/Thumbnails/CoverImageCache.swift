...
/// 使える値型）でキー付けする形に落とし込んでいる。このキャッシュは「自動生成
/// したサムネイルを PNG で保存するだけ」——元画像の形式を問わず、生成物を
/// 一貫した形式で保存すればよいため。
///
/// **ユーザー指定カバーの複製（TH-04、元の拡張子を保つ）はここに置かない。**
/// 置き場所は `UserCoverStore`。こちらは上限 [IV-09] で削られ「キャッシュを
/// 空にする」で全部捨てられる領域なので、混ぜると元画像が既に無いカバーを
/// 巻き添えで失う [CV-08]。
