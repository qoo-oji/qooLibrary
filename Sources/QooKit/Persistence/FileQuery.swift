...
public struct FileQuery: Sendable, Hashable {
    ...
    ///
    /// **`grouping` と両方立ててよい。優先順（シリーズが先）は永続化層が持つ。**
    /// 外側の一覧はシリーズで畳み、スタックを開いた巻一覧の中で重複を畳む
    /// ——という役割分担 [VM3-01 の設計判断] は、呼び出し側が片方を落とすのでは
    /// なく永続化層の優先順で満たす。**先に落とすと、シリーズ名を 1 件も持たない
    /// ライブラリで重複グループ化が黙って死ぬ** [VM3S-12]——事前確認 [VM3S-04] が
    /// 畳まずに返すのに、呼び出し側は畳んだつもりでいるため。
    ///
    /// **どちらで畳んだかは `FilePage.groupedBy` が返す**（希望ではなく結果）。
    ///
