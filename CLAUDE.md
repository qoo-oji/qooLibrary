...
サークル名 1 件（実蔵書に 19 件現れるもの）・作品名 2 件が入っていた
ことが分かった。フェーズ 1 で混入し、**既に push 済み**だった（それぞれ 6 / 4 / 1
コミット）。合成名（`98765架空社` `サンプルプレビュー` `バグベア` 等）へ
置き換え、`Scripts/check-private-data-leak.swift` を新設した（変異検証済み）。

**過去のコミットには残っている。**現在の作業ツリーから消えただけで、履歴の
書き換え（`git filter-repo` 等）と force-push は行っていない——リポジトリの
歴史を書き換える判断はユーザーのものだから。
...
教訓が 2 つある:
  ...
  - **最初の漏洩走査は NFD を見落としていた。**コーパスの 75.5% は NFD なので、
    NFC の検索語では濁点を含む名前に当たらない（濁点を含む実名が 0 件と出て気づいた）。
    さらに最初の検査は**まるごとのトークンしか見ておらず**、実在名の一部
    （長い作品名の中に含まれる短いカタカナ語）を取りこぼした——変異検証で分かった。
    **走査そのものを変異で試すこと。**
