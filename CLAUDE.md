...
| 2-13 ラベル保管庫の整理ウインドウ | **完了**（2026-08）。`LAW-01〜03` の 2 ペイン。**新しいコマンドは 1 つも書いていない**（既存の `SetLabelArchivedCommand` / `DeleteLabelsCommand`）。**ファイル保管庫 [FAW-01〜05] は 2-11 のまま**。詳細は下記「ラベル保管庫の整理ウインドウ」節と 15章 §15.3。**実機検証済み** |
...
##### 意図的に残した穴（忘れずに）
  ...
  - **`LibraryRepository.unregister(keepLabels:)` は `keepLabels` を無視する。**
    ...
    変えること [RG-06]。環境設定「リセット」タブの削除も同じ経路を通る。
...
——つまり実機では観測できない。`managedFile.isArchived` を直に書いてテストで固定した。
...
| ~~2-13~~ | ~~ラベル保管庫の整理ウインドウ [LAW-01〜03]~~ | **完了**（2026-08、下記「ラベル保管庫の整理ウインドウ」節）。ファイル保管庫 [FAW-01〜05] は 2-11 のまま |
| 2-14 | 未解決／孤立ファイルの整理ウインドウ [UR-01〜06][OR-01〜05] | 2-2 で孤立が日常的に発生するのに件数しか分からないまま。ただし `unresolvedFile` テーブルに誰も書いていないので、**2-7 の記録側から作る必要がある**（見た目より大きい）|
...
| 2 | `.qooarchive` 配下のサイドカー探索 [FA-17] は 2-11（ファイル保管庫）のまま |
| 3 | アーカイブ状態の表示 [DT-11] も 2-11。アプリの関連付け [DT-07] は表示の置き場所を決めていない |
...
| 2 | ファイル保管庫の整理ウインドウ [FAW-01〜05] は 2-11 のまま。`managedFile.isArchived` の器はあるが、ファイルを送る導線が無い |
...
| 2-14 | 未解決／孤立ファイルの整理ウインドウ [UR-01〜06][OR-01〜05] | 2-2 で孤立が日常的に発生するのに件数しか分からない。ただし `unresolvedFile` テーブルに誰も書いていないので、**2-7 の記録側から作る必要がある**（見た目より大きい）|
| 2-7 の残り | `RC-01〜16` の再計算ポリシー、`AL-30〜34` の救済フロー | 2-14 の前提 |
| 2-11 | ラベル／ファイル／フォルダアーカイブ [LA-01〜09][FA-01〜17] | ラベル側は済んでいる。**ファイル保管庫を作れば §15.4 の整理ウインドウが 2-13 と同じ形で足せる** |
...
| 2-11 | ファイル／フォルダ保管庫 [FA-01〜17][FDA-01〜05] | **2-9 が前提だったので着手できるようになった。** ラベル側は 2-6/2-13 で済んでおり、`managedFile.isArchived` の器もある。作れば §15.4 の整理ウインドウが 2-13 と同じ形で書ける |
...
| # | 内容 | なぜ今か |
|---|---|---|
| 2-11 | ファイル／フォルダ保管庫 [FA-01〜17][FDA-01〜05] | **2-9 が前提だったので着手できる。** ラベル側は 2-6/2-13 で済み、`managedFile.isArchived` の器もある。作れば §15.4 の整理ウインドウが 2-13 と同じ形で書ける |
| ~~2-14~~ | ~~未解決／孤立ファイルの整理ウインドウ~~ | **孤立側は完了**（2026-08、画面では「見つからないファイル」と呼ぶ）。「`unresolvedFile` に誰も書いていないので 2-7 から要る」が当てはまるのは**未解決側だけ**だった——孤立側は `state == .orphaned` を読むだけで着手できた |
...
- **登録ルート行は 2 つある**（通常と縮退）。項目を足すときは配線が 2 か所に
  要る——これで 5 度目。
- **2-11（ファイル保管庫）を作るときは、この節の形をそのまま使える。**
...
| # | 内容 | なぜ今か |
|---|---|---|
| 2-11 | ファイル／フォルダ保管庫 [FA-01〜17][FDA-01〜05] | **道具が揃った。**`ManagedFileSnapshot` / `restoreFiles` / 2 ペインの形をそのまま流用でき、§15.4 の整理ウインドウまで続けて書ける |
...
| # | 内容 | なぜ今か |
|---|---|---|
| 2-11 | ファイル／フォルダ保管庫 [FA-01〜17][FDA-01〜05] | **道具が揃っている。**`ManagedFileSnapshot` / `restoreFiles` / 2 ペインの形をそのまま流用でき、§15.4 の整理ウインドウまで続けて書ける |
| 2-7 の残り | `unresolvedFile` への記録、`AL-30〜34` の救済フロー、`RC-01〜16` | **2-14 の未解決側（§15.6）の唯一の前提。** 孤立側が片付いたので、残るのはこちら |
...
| # | 内容 | なぜ今か |
|---|---|---|
| 2-11 | ファイル／フォルダ保管庫 [FA-01〜17][FDA-01〜05] | **道具が揃っている。**`ManagedFileSnapshot` / `restoreFiles` / 2 ペインの形をそのまま流用でき、§15.4 の整理ウインドウまで続けて書ける。**未解決一覧が `isArchived` を除外する経路も、これが入って初めて実際に効く** |
| 2-18 | 重複ファイルの比較・削除 [DU-01〜29] | 2-9 が前提。[VM2-16] の重複グループ表示もここ |
