...
| 2-13 | ラベル保管庫の整理ウインドウ [LAW-01〜03] | **2-6 で保管庫へ送れるようになった**ので、送った先を一覧して戻す画面の必要度が上がった。器（`isArchived`）も導線も揃っており、要るのは 2 ペインの画面だけ |
| 2-14 | 未解決／孤立ファイルの整理ウインドウ [UR-01〜06][OR-01〜05] | 2-2 で孤立が日常的に発生するのに件数しか分からないまま。ただし `unresolvedFile` テーブルに誰も書いていないので、**2-7 の記録側から作る必要がある**（見た目より大きい）|
| 2-9 | ライブラリ表示モード [VM-10〜16] | 中央ペインを DB の一覧で描く経路がまだ無い。4 つの中では最も大きい |
...
| # | 内容 | なぜ今か |
|---|---|---|
| 2-13 | ラベル保管庫の整理ウインドウ [LAW-01〜03] | 2-6 で保管庫へ送れるのに、送った先を見る手段が無い。器も導線も揃っており、要るのは 2 ペインの画面だけ——**4 つの中でいちばん小さい** |
