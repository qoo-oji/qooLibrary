...
60. ~~**⌘A は入力欄ではなくウインドウへ行く。**~~ → **製品側を直した**
    ...
    ⌘A で欄の中身を選び直してよい。
61. **実ホームを開くと TCC の許可ダイアログ（デスクトップ）が出る。**
    答えると利用者の権限設定が変わるので、**答えずに `killall
    UserNotificationCenter` で消し**、`qoo.favorites.visible.desktop` を
    一時的に false にして出ないようにした（検証後に削除して元へ戻す）。
...
96. **フォーカスモードでは「ツール呼び出しで終わるターン」は完全に無言**になる。
