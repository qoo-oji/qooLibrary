...
#### 次にやること
  ...
  2-5 ライブラリタイプ・テンプレート（DB 側の編集 UI。テンプレート定義自体は完了）
  2-6 ラベルグループ・ラベルの管理、配色（配色のアルゴリズムは実装済み [MT-13]）
  2-7 自動ラベル付与、再計算ポリシー（付与自体はスキャンに実装済み。RC-01〜16 の
      ポリシーと未解決ファイルの救済フローが残り）
  2-8 以降は UI が主。着手前に 14章・15章を読むこと。

**実機検証は未実施。**フェーズ 2 はまだ UI に接続していないため（アプリからは
DB もスキャンも呼んでいない）、`xcodebuild` が通ることだけを確認している。
