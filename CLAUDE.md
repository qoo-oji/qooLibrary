...
**フェーズ 0（基盤検証）完了。フェーズ 1（ファイルマネージャー）完了（1-1〜1-17 すべて完了。1-12 は実装可能な範囲を実装済み）。フェーズ 2（ライブラリマネージャー）進行中 — 2-1 / 2-2 / 2-3 / 2-4 / 2-6 / 2-8 / 2-9 / 2-10 / 2-13（ラベル側）/ 2-14（孤立側）完了。**2-9（ライブラリ表示モード）は 2026-08 に残り 3 件を片付けて閉じた**——中央ペインを DB の一覧として描け、一覧にカバー画像が出て [IV-02]、タイトル・シリーズ名で検索でき [SR-03]、ブックフォルダに印が付く [IF-17][IF-18]（**すべて実機検証済み**。下記「ライブラリ表示モード」節と「2-9 を閉じた 3 件」節）。**2-10（右ペインの評価・ラベル・タイトル・カバー）は 2026-08 に第 3 段まで終わって閉じた**（3 段とも実機検証済み。カバーの D&D [CV-03] だけ未確認）。**2-1（同一性判定）は 2026-08 に 2 度改訂した**——差し替えを走査後に一括で確認する形にし [ID-05][ID-09〜12]、続けて**どこまで黙って同じファイルとみなすかをライブラリ設定から選べる**ようにした [ID-13〜15]。**既定は「名前が同じなら確認しない」**なので、[ID-03]③ が自動で紐づかないことを前提にしたコードがあれば見直すこと（下記の 2 節）。**2-8（ラベルフィルタ）で左ペイン下半分が実際に動くようになった**——ラベルで中央ペインを絞れる。2-12（ライブラリ設定ウインドウ）は第 1 段・第 2 段とも完了。2-16（JSON 入出力）は第 1 段が完了。**2-19（埋め込みメタデータの読み取り）は完了**——`ComicInfo.xml` / EPUB / PDF から著者・タイトル・シリーズ・巻数を読み、ファイル名由来の値より優先する。アプリとの結線が済み、実機のサンドボックス下で「登録フォルダを有効化（設定を確認・調整し、実ファイル名でのプレビューを見てから決められる）→ 実ファイルを DB へ取り込み → ラベル自動付与 → 設定を編集して再スキャンで反映 → JSON へ書き出し → ライブラリを削除 → 取り込みで復元」まで動作を確認済み。**
...
| 2-14 | 未解決／孤立ファイルの整理ウインドウ | **孤立側は完了**（2026-08、下記「見つからないファイルの整理ウインドウ」節）。未解決側は 2-7 の記録側が前提 |
