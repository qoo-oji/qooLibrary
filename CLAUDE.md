...
| 高 | **NV8-03**（`EDEADLK` の再現）| 実アプリの並行構造（`actor`＋同時実行スロット）で再現するかを測る。**NV6-01 を入れた後の構造で測ること**。結果次第で `NSFileCoordinator` の要否が決まる。**この機にはクラウドストレージが無いので、まず環境が要る** |
