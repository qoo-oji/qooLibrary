...
**フェーズ 0（基盤検証）完了。フェーズ 1（ファイルマネージャー）着手済み（1-1〜1-3 完了）。**
...
### フェーズ 1（`17_実装ロードマップ.md` §17.3、1-1〜1-3 完了・1-4 以降未着手）
...
- **未着手**（1-4 以降）: フォルダツリー（左ペイン）、`FileOperationService`、ドラッグ＆ドロップ、圧縮展開 UI、Undo 基盤、環境設定、通知基盤、診断ログ、右ペインの詳細情報（現状 1-2 の検証 UI が仮置き）等。
...
フェーズ1 ファイルマネージャー  Finder 代替として日常使用できる状態             ← 進行中（1-1〜1-3 完了）
