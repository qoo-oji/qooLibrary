...
**別件だが引き継ぐこと: `DirectoryWatchIntegrationTests` に既存のフレークがある。**
`externalRemovalOfTheWatchedDirectoryReachesTheObservation` が **Release で
4 回に 1 回ほど落ちる**（Debug でも稀に）。`git stash` して **HEAD でも落ちる**
ことを確認済みで、1-16b の変更とは無関係。原因は `waitForStreamStartup()` が
**500 ms の固定 sleep** で、並列実行時に FSEvents の登録が間に合わないと
`rmdir` を取りこぼし 10 秒待ちで落ちる。直すなら固定 sleep をやめ、
**プローブ用の変更が実際に観測できるまで待ってから本題に入る**形にする。
