...
**フェーズ 0（基盤検証）完了。フェーズ 1（ファイルマネージャー）完了（1-1〜1-17 すべて完了。1-12 は実装可能な範囲を実装済み）。フェーズ 2（ライブラリマネージャー）進行中 — 2-1 / 2-2 / 2-3 / 2-4 / 2-6 / 2-8 / 2-9 / 2-10 / 2-11 / 2-13 / 2-14 完了。**

**2-11（ファイル／フォルダ保管庫）と §15.4 の整理ウインドウは 2026-08 に完了**
——蔵書を捨てずに `.qooarchive` へ外し [FA-01〜17][FDA-01〜05]、しまった先を
...
| 2 | **OR2-05（通知履歴のバッジ）は未。** 通知履歴 [NW-01〜08] もステータスバーのバッジも未実装で、NT-06 が「通知履歴のバッジに統合する」と定めている以上、先に作るのはそちら |
...
| — | 通知履歴 [NW-01〜08] | OR2-05 のバッジ [NT-06] がこれを待っている |
...
| 3 | **UR2-02 の通知履歴への記録は未**（`NW-01〜08` が未実装）。導線は走査結果のシートに出してある |
...
| 行の表示 [FAW-02][FAW-05] | ファイル名 ＋ しまった日時 ＋「ラベル 3 件」＋ 右端に「保管庫から戻す」|
| 並べ替え [FAW-05] | 「名前順／しまった日時順」が切り替わり、**区画をまたがない** |
...
**① 選択すると 2 行目が読めなくなっていた。** 行はファイル名と「しまった日時
...
| — | 通知履歴 [NW-01〜08] | **待っているものが 2 つある**——OR2-05 のバッジ [NT-06] と UR2-02 の記録 |
