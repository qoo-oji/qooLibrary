...
#### 未実装: フォルダツリーのキーボード操作（3 通り試して、いずれも実機で動かなかった）

**着手する前にこの節を読むこと。** 同じ道を 3 回試して 3 回とも失敗している。

書けた範囲（コミットしていない。作業ツリーからは戻してある）:

- 子の一覧を**ペインが持つ**ようにして（各行が `loadedChildren[node.id]` へ書き込む）、表示順のまま平坦化した行の配列を作る。ここまでは素直に書ける。
- その配列で「↑ ↓ で 1 行動く」「→ で展開／最初の子へ」「← で折りたたみ／親へ」も書ける。

**動かなかったのは配線（キーが届かないこと）。** 実機で確定した事実:

| 試したこと | 結果 |
|---|---|
| `List` に `.focusable()` ＋ `.onKeyPress(.upArrow/.downArrow)` | **↑ ↓ が一度も届かない。** `List` の中身は `NSOutlineView` で、矢印を自分の行移動として先に処理してしまう |
| `List(selection:)` を持たせて AppKit の行移動に乗る（行の label に `.tag()`）| **最初の ↓ で先頭行（Macintosh HD）へ飛び、以後動かなくなった。** `DisclosureGroup` で組んだツリーではタグと行の対応が期待どおりにならない |
| `NSEvent.addLocalMonitorForEvents` で ↑ ↓ を横取り（`BackForwardGestureSupport` と同じ手口）| **監視が働かない。** `@FocusState` を条件にすると真にならず、ファーストレスポンダ（`NSOutlineView`）を条件にしても動かなかった |

診断に使えた事実:

- 行をクリックすると `AXFocusedUIElement` は **`AXOutline`** になる（＝ AppKit 側のフォーカスはツリーが持っている）一方、SwiftUI の `@FocusState` は真にならない。**この 2 つは一致しない。**
- クリック自体は届いている（別の行をクリックするとウインドウタイトルが変わる）。届いていないのは**キー入力だけ**。
- ← → も届かなかった。当初 1 回だけ「← で親へ移動した」ように見えたが、ウインドウがまだ現れていない状態での計測で、整えて再現したところ再現しなかった。**信用できない観測だった。**

**次に試すなら**: SwiftUI の `List` ＋ `DisclosureGroup` という組み合わせをやめる方向。`OutlineGroup` ＋ `List(selection:)` へ組み替えるか、`NSOutlineView` を `NSViewRepresentable` で直接使う。**キーの横取りで解決しようとしないこと** — 試した範囲では、`NSOutlineView` がフォーカスを持っている限り SwiftUI 側にキーが渡ってこない。

**教訓**: 「フォーカスがある」は SwiftUI と AppKit で別々の話。`@FocusState` が偽でも `AXFocusedUIElement` は埋まっている、という状態が普通に起こる。キーが届かないときは、まず `AXFocusedUIElement` を実機で見て**どちらの世界がキーを受け取っているか**を確かめること。
...
| 1-16b | **ネットワークボリュームの扱い**（ゴミ箱が使えない場所での削除、変更検知）| 未着手（事実の記録のみ、`08_インフラ_ファイル操作.md` §8.4「ゴミ箱を持てないボリューム」）|
