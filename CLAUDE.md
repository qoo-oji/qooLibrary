...
**フェーズ 0（基盤検証）完了。フェーズ 1（ファイルマネージャー）完了（1-1〜1-17 すべて完了。1-12 は実装可能な範囲を実装済み）。フェーズ 2（ライブラリマネージャー）進行中 — 2-1 / 2-3 / 2-4 完了、2-2 は結線を残す。2-12（ライブラリ設定ウインドウ）は第 1 段が完了。アプリとの結線が済み、実機のサンドボックス下で「登録フォルダをライブラリとして有効化 → 実ファイルを DB へ取り込み → ラベル自動付与 → 設定を編集して再スキャンで反映」まで動作を確認済み。**
...
| 2-5 ライブラリタイプ・テンプレート | テンプレート定義（JSON・展開・登録時のコピー）は**完了**。**ライブラリ側の設定編集も完了**（下記「ライブラリ設定ウインドウ」）。残りはユーザー定義テンプレートの複製・保存・JSON 入出力 [LT-02][LT-06] と `LT-04`（後から適用する確認ダイアログ）・`LT-10〜17`（改訂の差分適用）|
...
| # | 内容 |
|---|---|
| 1 | **有効化ダイアログのプレビュー**［ユーザー判断で採択済み］。`LibrarySettingsDraft.compiledSnapshot()` をそのまま使えるので、新しい機構は要らない |
| 2 | **白紙（カスタム）からの有効化**［同上］ |
| 3 | **ユーザー定義テンプレート**（複製・保存・JSON 入出力）[LT-02][LT-05][LT-06] |
| 4 | 左ペインからの登録・削除、テンプレートの適用・変更 [LS-03]、差分ビュー [LT-13] |
| 5 | ラベルグループの色編集 [LE-10]、ラベル一覧 [LE-01〜12]（2-6 の編集ウインドウ） |
