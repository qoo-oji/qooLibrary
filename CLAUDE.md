...
| ~~2-9 ライブラリ表示モード~~ | **完了**（2026-08、下記「ライブラリ表示モード」節）。中央ペインを DB の一覧で描く経路ができた。実機検証済み |
...
| # | 内容 |
|---|---|
| 1 | **[IV-02] 一覧のカバー画像**。サムネイルは従来どおり `ThumbnailService`（先頭画像）で、**ユーザー指定カバー [CV-06] とサイドカーを見ない**。解決は `CoverEditorModel` に揃っているので、そこから呼べばよい |
| 2 | **[SR-03] タイトル・シリーズ名での検索**。`searchKey` 列は**ファイル名の stem からしか作られていない**。満たすには `searchKey` の作り方を変え、タイトル編集時（`setFields`）にも更新する必要がある |
| 3 | **[IF-17][IF-18] ブックフォルダのフォルダ表示モード側**（インジケータ・開く既定）。`isBookFolder` は DB にあるが UI へ出していない。**スコープに含めると答えたのに未着手** |
