...
- **第 2 部（EPUB / PDF / ComicInfo.xml のメタデータ読み取り）は未着手。**
  調査結果だけ 05章 §5.7 に残してある——**ComicInfo の巻数は Komga が `Number`、
  Kavita が `Volume` で真逆**。公式スキーマ（Anansi Project）は `Number` =
  シリーズ内の通し番号、`Volume` = US コミック特有の「版」で Komga 側が忠実。
  食い違ったときは**ファイルごとに確認（「以降すべてに適用」つき）**［ユーザー判断］。
