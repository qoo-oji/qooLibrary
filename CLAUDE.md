...
#### 実機で確かめたこと（実蔵書には触れていない）

実ストアが**ライブラリ 0 件**だったので、起動しても走査は始まらない状態で
確認した: `v3` → `v4_fsEventsCheckpoint` の移行が走り、`library` と
`temporaryFolder` の両方に列ができる／`実体への追随を開始: 監視 0 ライブラリ`
と `…停止し、差分の起点を保存した` がログに出る／CPU 0% で正常終了する。

**実ライブラリを有効化しての通し確認は未了**（登録と有効化に GUI が要る）。
