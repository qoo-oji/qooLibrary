...
| # | 内容 |
|---|---|
| 1 | **確認ダイアログが他のウインドウの下に隠れることがある**［実機で 1 度発生］。設定ウインドウを開いた状態で通知から開いたとき、`CGWindowList` の layer が 8（モーダル）ではなく **0** だった。`AXRaise` で 8 になり操作できた。再現条件を詰めきれていない——**ユーザーの実機で「通知 → 巻数を確認…」が前面に出るか確認したい** |
| 2 | **「今後このライブラリでは同じ判断を使う」のチェック操作が未検証**（合成イベントでは押せなかった。動作自体は `aSettledLibrarySettingResolvesConflictsSilently` が固定している）|
| 3 | **有効化ウインドウのプレビューはメタデータを読まない** [LE2-16]。設定を変えるたびに最大 500 件を開く費用に見合わないため。**メタデータを持つファイルではプレビューと実際の値が食い違う**——将来、対象を数件に絞って読む案はある |
| 4 | `Penciller` 等の他の役割は読まない [EM-28]。`@author` に対応させるのは `Writer` のみ |
| 5 | RAR での実測は未了（`ArchiveBackendRegistry` 経由で読める設計だが、実 RAR に `ComicInfo.xml` を入れた標本を作っていない）|
