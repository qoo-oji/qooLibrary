...
**フェーズ 0（基盤検証）完了。フェーズ 1（ファイルマネージャー）完了（1-1〜1-17 すべて完了。1-12 は実装可能な範囲を実装済み）。フェーズ 2（ライブラリマネージャー）進行中 — 2-1 / 2-3 / 2-4 完了、2-2 は結線を残す。アプリとの結線が済み、実機のサンドボックス下で「登録フォルダをライブラリとして有効化 → 実ファイルを DB へ取り込み → ラベル自動付与」まで動作を確認済み。**

> **フェーズ 2 に着手する人へ**: まず直下の「フェーズ 2 の記録」節を読むこと。
> **永続化が SwiftData から SQLite(GRDB) へ変わっており**、要件定義書も v2.10 に
> 改訂されている（ファイル名も `qooLibrary_要件定義書_v2.10.md`）。
...
| 3 | **`label` と `labelGroup` だけ `AUTOINCREMENT`**。ウインドウ状態復元がラベルフィルタの選択を保持するため、rowid が再利用されると復元後に別のラベルが選ばれる。`managedFile` は付けない（生存中の参照は外部キー、履歴はパスを非正規化して持つ）|
...
2. **「登録解除」が `library` 行を消していなかった。** 登録だけ消えて
   ...
   **完全削除による強制解除（`DeletePermanentlyCommand`）にも同じ穴があった**
   ...
   （`unregister` は `keepLabels` をまだ見ずに連鎖削除する）。ラベル保管庫
   [RG-06][2-11] が入るまで「保持する」を選べないので、**何が失われるかを
   伝える確認ダイアログ**を挟むようにした。
...
#### この結線で新たに分かった、次の人向けの注意
  ...
  - **`ScanEngine.Mode.incremental` をまだ `LibraryWatcher` に繋いでいない。**
    繋ぐ前に「`.incremental` が差分パスを使っていない（ライブラリ全体を列挙する）」
    という既知の穴を先に塞ぐこと——さもないと**ファイルが 1 つ変わるたびに
    ライブラリ全体の列挙が走る。**
  - `VolumeMonitor` も未接続。繋ぐと `isOnline` が動き出すが、上記 2 の砦は
    それとは独立に残すこと。
  - `VolumeMonitor.events` の `Task { await self.register(...) }` は
    「不要な await」警告を出す（良性——アクター内で既に隔離済み）。ただし
    **登録が非同期なので、`events` を取った直後に `start()` すると初期の
    イベントを取りこぼしうる**。購読側で再突き合わせを併用すること。

##### 未着手（ロードマップ順）

| # | 状態 |
|---|---|
| 2-2 の残り | **`LibraryWatcher` / `VolumeMonitor` のアプリへの結線**（上記の注意を読んでから）|
| 2-5 ライブラリタイプ・テンプレート | テンプレート定義（JSON・展開・登録時のコピー）は**完了**。残りは編集・複製の UI と `LT-04`（後から適用する確認ダイアログ）・`LT-10〜17`（改訂の差分適用）|
| 2-6 ラベルグループ・ラベル管理 | 配色のアルゴリズム [MT-13] とリポジトリ（`ensureLabel`/`merge`/`rename`/`setArchived`/`setPinned`）は**完了**。残りは編集ウインドウ [LE-01〜12] |
| 2-7 自動ラベル付与・再計算 | **付与自体はスキャンに実装済み**（`ScanEngine.applyLabels`、`replaceAutoLabels` が `manual`/`manuallyRemoved` を守る [RC-04]）。残りは `RC-01〜16` の再計算ポリシー、`AL-30〜34` の未解決ファイル救済フロー、`unresolvedFile` テーブルへの記録（**テーブルはあるが誰も書いていない**）|
| 2-8 以降 | UI が主。着手前に 14章・15章を読む。**左ペイン下半分の「ラベルフィルタ」は今も「まだ利用できません」のプレースホルダ**——DB にラベルが入るようになったので、次に効果が見えるのはここ |
...
##### 意図的に残した穴（忘れずに）
  ...
  - **`LibraryRepository.unregister(keepLabels:)` は `keepLabels` を無視する。**
    ...
    前に「ラベル・評価が失われる」確認を挟んである**——ラベル保管庫（2-11）が
    入ったら `true` の経路をそこへ繋ぎ、確認を「保持する／破棄する」の選択へ
    変えること [RG-06]。
  - **`ScanEngine` の `.incremental` は差分パスを使っていない。**ライブラリ全体を
    列挙してから収束させる（結果は正しいが遅い）。`LibraryWatcher` が渡す
    `relativePaths` を使ってフォルダスコープへ絞るのが本来。**`LibraryWatcher` を
    結線する前に必ずここを塞ぐこと**——さもないとファイルが 1 つ変わるたびに
    ライブラリ全体の列挙が走る。
  - **`library.isOnline` を更新する経路がまだ無い**（常に真）。`VolumeMonitor` を
    結線するときに VD-01〜06 で駆動する。**ただし `ScanEngine` の根の同一性検査は
    それとは独立に残すこと**（上記「結線の過程で見つけて直した既存の欠陥」②）。
  - `temporaryFolder` / `pendingMove` / `notificationRecord` / `operationLog` /
    `appAssociation` の各テーブルはスキーマだけあって実装が無い（フェーズ 2 後半・3）。
...
- **検証は必ず使い捨てボリュームで。** 現在地を確かめずに「新規フォルダ」を実行し、
  ユーザーの実フォルダ（`<登録フォルダ>`）に `ProbeFolder` を作ってしまった（削除済み）。
...
確認済み（`/Volumes/PRO-G40/<登録フォルダ>` を開いた状態のスクリーンショットで
...
作業を始める際は、対象領域の仕様書（下記 §2 の一覧）を該当箇所だけ `Read` してから着手する。仕様書は合計 18 ファイルあり、全部を毎回読み込む必要はない。要件定義書本体は `docs/Requirements/qooLibrary_要件定義書_v2.10.md` にある（約 2,900 行）。矛盾したときの優先順位は §1 参照。
...
対応する要件定義書は「qooLibrary 要件定義書 v2.10」（約 1,053 要件 ID、`docs/Requirements/qooLibrary_要件定義書_v2.10.md`）。実装仕様書 `docs/Specifications/` はこれを実装に落とし込んだもので、要件 ID を `[XX-00]` の形式で併記している。両者が矛盾した場合は要件定義書が最上位だが、実務上は日常的には `docs/Specifications/` の記述に従い、矛盾を見つけたら仕様書側を直す（無断でコードだけ仕様と異なる実装にしない）。要件定義書は分量が大きいため、要件 ID の一次情報や仕様書に説明のない詳細を確認したいときだけ該当セクションを開く。
...
要件定義書本体（`docs/Requirements/qooLibrary_要件定義書_v2.10.md`）はこれらの一次情報。仕様書に説明のない詳細や、要件 ID そのものの文言を確認したいときに開く。
