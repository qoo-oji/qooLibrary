...
| 2-10 右ペインのラベル・評価・カバー | **第 1 段（評価 RA-01〜RA-08）が完了**（2026-08、下記「右ペインの評価」節）。**インスペクタが DB を触る経路が通った**ので、残り（ラベル設定 [RL-01〜07]・カバー画像 [CV-02〜08]・タイトル編集 [RP-10〜12]）は同じ形で足せる。あわせて**「DB 操作の Undo」（2-15）の 1 つ目**を作った |
...
| # | 内容 | なぜ今か |
|---|---|---|
| 2-6 の残り | ラベルグループ・ラベルの編集ウインドウ [LE-01〜12] | リポジトリと配色は済んでいる。**ラベルフィルタから改名・統合・保管庫へ送る導線が無い**のは 2-8 の時点から変わっていない |
| 2-10 の残り | ラベル設定 [RL-01〜07]・カバー画像 [CV-02〜08]・タイトル編集 [RP-10〜12] | **土台が通ったばかりで、同じ形で足せる**（上記）。ただしラベル設定は 2-6 の編集ウインドウと重なる部分があるので、どちらを先にするかは決めてから |
...
| 4 | **読めないアーカイブを選ぶと「含まれるファイル数」のスピナーが止まらない**［実機検証で気づいた、**評価とは無関係の既存の挙動**］。`computeContainedCounts` が `nil` を返すと `containedCounts` が埋まらないまま。検証で使った 1 バイトの偽 `.cbz` で踏んだもので、実ファイルでは起きない——が、壊れたアーカイブでは同じことが起きる |
