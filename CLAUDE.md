...
| ~~2-9 ライブラリ表示モード~~ | **完了**（2026-08）。中央ペインを DB の一覧で描く経路ができ、**残り 3 件（IV-02 / SR-03 / IF-17・18）も片付けて閉じた**。下記「ライブラリ表示モード」節と「2-9 を閉じた 3 件」節。**後者は実機検証が未了** |
...
| # | 内容 |
|---|---|
| 1 | **実機検証が未了。**下記の手順で行うこと。特に ①カバーがユーザー指定 → サイドカー → 先頭画像の順で出るか ②タイトルで検索して出るか ③ブックフォルダに印が出て、設定を切り替えると開き方が変わるか |
