...
**フェーズ 0（基盤検証）完了。フェーズ 1（ファイルマネージャー）完了（1-1〜1-17 すべて完了。1-12 は実装可能な範囲を実装済み）。フェーズ 2（ライブラリマネージャー）進行中 — 2-1 / 2-2 / 2-3 / 2-4 完了。**
...
**まずこの順で読む**: ① 本節 ② `Spikes/README.md` の「T-03 / T-04」と「T-05」
（設計を決めた実測値）③ `docs/Specifications/07_永続化_スキーマとリポジトリ.md`
④ 触る領域の章。

##### 最大の空白: **アプリと繋がっていない**

`QooPersistence` / `ScanEngine` / `LibraryWatcher` / `VolumeMonitor` はどれも
**アプリ層から一度も呼ばれていない**。`qooLibraryApp` は今もフェーズ 1 の
`RegisteredFolderStore`（`registeredFolders.json`）だけを見ている。したがって
**フェーズ 2 の成果はまだ実機で 1 度も動いていない**——`swift test` と
`xcodebuild` が通ることしか確認していない。本プロジェクトの品質基準（実機で
確かめる）を満たすには、次のどこかで結線が要る。

結線するときの要点:

  - `library.uuid` は**フェーズ 1 の登録フォルダ ID を引き継ぐ**ためにある
    （07章 §7.3）。`registeredFolders.json` の `id` をそのまま入れれば、
    環境設定の「起動時に開くフォルダ」・`NavigationRoot.registeredFolder(id:)`・
    ウインドウ状態復元がそのまま動く。**新しい UUID を振ってはいけない。**
  - ストアの置き場所は `~/Library/Application Support/qooLibrary/qooLibrary.sqlite`。
    `QooDatabase.open(at:beforeMigration:)` の `beforeMigration` で JSON
    スナップショットとストア複製を取る [MG-10]。
  - 開発機には登録済みライブラリが 6 件・実ファイル 2,346 件ある。**初回スキャンは
    実データに当たる**ので、`.orphaned` の扱いとオフライン判定を先に確認すること。
...
| 2-8 以降 | UI が主。着手前に 14章・15章を読む |

##### 意図的に残した穴（忘れずに）

  - **`unresolvedFile` テーブルに誰も書いていない。**`ScanSummary.unresolvedNames`
    は数えるだけ。2-7 で `UnresolvedFileQueue` [UR2-01] を作るときに繋ぐ。
  - **`ScanSummary.bookFoldersReleased` を誰も通知していない** [IF-05 は「通知する」
    と定めている]。`NotificationRouter` へ繋ぐのは UI 結線のとき。
  - **`LibraryRepository.unregister(keepLabels:)` は `keepLabels` を無視する。**
    どちらでもライブラリ行を消す（連鎖でラベルも消える）。ラベル保管庫（2-11）が
    入ったら `true` の経路をそこへ繋ぐ [RG-06]。
  - **`ScanEngine` の `.incremental` は差分パスを使っていない。**ライブラリ全体を
    列挙してから収束させる（結果は正しいが遅い）。`LibraryWatcher` が渡す
    `relativePaths` を使ってフォルダスコープへ絞るのが本来。
