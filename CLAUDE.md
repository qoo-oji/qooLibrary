...
- `sudo` にパスワードが要るため**こちらからは実行できない**。ユーザーに
  スクリプトを渡して走らせてもらう。
...
1. **ネットワーク共有を実際に開いて固まらないこと**（1-16b の本丸）。一覧・
   ツリーの展開・再帰検索・インスペクタの集計のいずれも非同期化した
2. **同一共有内の D&D が移動になること**。従来はコピーになっていた（NV3-01 の
   穴）ので、**挙動が変わる**。4GB ならサーバ側の改名 1 回で済むはずのものが
   全バイトの往復になっていた
3. ネットワークフォルダをライブラリ／テンポラリに登録したときの警告表示 [NV-87]
4. ネットワークボリュームの右クリック・ツリー行のボタン・File メニューが
   「接続解除」になっていること [NV-96]
5. ゴミ箱を持たない場所での削除が、アプリ自身の確認シートを経て完全削除へ
...
| 1-16b | **ネットワークボリューム・File Provider の扱い**（ゴミ箱・同一性・変更検知・実行モデル・性能縮退）| **実測完了・主要項目まで実装済み**（NV6-01/02・NV3-01・NV-92・NV-90・NV-87・NV-96・NV-97・NV-99）。残りは §8.11.11／§8.11.13。 SMB 3 系統（Samba/QNAP・Apple・Windows）と iCloud で測定し、`08_インフラ_ファイル操作.md` §8.11 を実測結果と決定事項で全面的に書き換えた。**記述には [実測]／[普遍]／[依存]／[文献] の札が付いているので必ず確認すること。** 残る未検証項目は §8.11.11 |
