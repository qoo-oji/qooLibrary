...
未着手のものは**どれも「測ってからでないと設計が決まらない」**:

| 優先 | 項目 | 着手時の勘所 |
|---|---|---|
| 高 | **NV8-04**（File Provider の同期先制約）| 400 文字のパス長・禁止文字。**ファイルシステムは通すのに同期だけが黙って失敗する**ので、既存の事前検査（`PathLimits`／`NameLengthLimit`／`FileNameValidation`）ではどれも捕まらない。**拒否ではなく警告**に倒すこと（誤って断らない原則）。本アプリは長く記号を含む名前を常態的に扱うので現実に踏む |
| 高 | **NV8-03**（`EDEADLK` の再現）| 実アプリの並行構造（`actor`＋同時実行スロット）で再現するかを測る。**NV6-01 を入れた後の構造で測ること** — §8.11.8 が「両者は一体で設計する」としており、先に入れた今がその状態。結果次第で `NSFileCoordinator` の要否が決まる |
| 中 | **NV-95**（アーカイブの読み取り往復数）| 特に Quick Look のカバー生成 [QL-03] は「中の先頭画像 1 枚」のために走査する。縮退が要るなら 09章 §9.6（DS-01〜07）の器に乗せる |
| 中 | **NV-94**（到達不能なパスを含む `FSEventStreamCreate`）| ブロックするならストリーム生成自体を `FileIO` へ移す |
| 中 | **NV-100**（N ウインドウで列挙 N 倍）| 「短時間だけ共有する」か「許容する」かを決める |
