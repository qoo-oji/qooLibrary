...
**フォルダツリーのキーボード操作**（Tab でのフォーカス移動・矢印での上下移動・← → での開閉）は手を付けていない。理由は他の項目と質が違うため: `FolderTreePane` は各行が `DisclosureGroup` で、子の一覧は**行ごとの `@State`** が持っている。キーボードで上下に動くには「今画面に見えている行の平坦な順序」が要るが、ペインはそれを知らない。素直に実装するには**ツリーの模型をペイン側が持つ構造変更**（子のキャッシュをペインへ移す）が要り、他の変更と混ぜると回帰の切り分けが難しくなる。着手するときは、その構造変更を単独で行うこと。
