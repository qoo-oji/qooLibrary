...
| 2-11 ファイル／フォルダ保管庫 | **完了**（2026-08）。§15.4 の整理ウインドウ [FAW-01〜05] も同時に閉じた。下記「ファイル保管庫」節。**実機検証だけ未了** |
...
| 3 | FA-11 の「ラベルフィルタ全 OFF のときだけアクセス可」は、**フィルタが効いている間は `.qooarchive` が `allowedChildNames` に入らない**ことで自然に満たされている（明示的な判定を足していない）。実機で確かめること |
