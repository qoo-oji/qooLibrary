...
- §8.11.8 に**検証計画（優先順位表）**を置いた。着手時はここから始める。特に上位 4 件
...
| P1-3 | 協調読み取り（`NSFileCoordinator`）| **不要と判明。実測で導入を見送った**（下記）|
...
| 1-16b | **ネットワークボリュームの扱い**（ゴミ箱が使えない場所での削除、変更検知、同一性、性能縮退）| 未着手。**事前調査は完了**（`08_インフラ_ファイル操作.md` §8.11、NV-10〜NV-88。ゴミ箱に限った論点は §8.4 の NV-01〜04）。**全項目が未実測の仮説**であり、着手時は §8.11.8 の検証計画から始める |
