...
`08_インフラ_ファイル操作.md` §8.11.11／§8.11.12 を正とする。未着手の主なもの:
**NV8-04**（File Provider の同期先制約——400 文字のパス長・禁止文字。ファイル
システムは通すのに同期だけが黙って失敗する）、**NV-100**（N ウインドウで列挙が
N 倍）、**EDEADLK の実アプリ並行構造での再現**（`NSFileCoordinator` の要否が決まる）、
NV-94／NV-95／NV-101。

**実機検証をお願いしたいもの**: アプリ層の非同期化（一覧・ツリー・検索・
インスペクタ・リンク解決・「フォルダへ移動…」）は自動テストが無い。起動して
一覧・ツリー・インスペクタが描画されることは確認済みだが、**ネットワーク共有を
実際に開いて固まらないこと**と、**同一共有内の D&D が移動になること**（従来は
コピーだった）は実機で確かめてほしい。
...
| 1-16b | **ネットワークボリューム・File Provider の扱い**（ゴミ箱・同一性・変更検知・実行モデル・性能縮退）| **実測完了・主要項目まで実装済み**（NV6-01/02・NV3-01・NV-92・NV-90・NV-87・NV-96・NV-97・NV-99）。残りは §8.11.11／§8.11.12。 SMB 3 系統（Samba/QNAP・Apple・Windows）と iCloud で測定し、`08_インフラ_ファイル操作.md` §8.11 を実測結果と決定事項で全面的に書き換えた。**記述には [実測]／[普遍]／[依存]／[文献] の札が付いているので必ず確認すること。** 残る未検証項目は §8.11.11 |
