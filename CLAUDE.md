...
**フェーズ 0（基盤検証）完了。フェーズ 1（ファイルマネージャー）完了（1-1〜1-17 すべて完了。1-12 は実装可能な範囲を実装済み）。フェーズ 2（ライブラリマネージャー）進行中 — 2-1 / 2-2 / 2-3 / 2-4 / 2-8 完了、2-10 は第 1 段（評価）完了。**2-8（ラベルフィルタ）で左ペイン下半分が実際に動くようになった**——ラベルで中央ペインを絞れる。2-12（ライブラリ設定ウインドウ）は第 1 段・第 2 段とも完了。2-16（JSON 入出力）は第 1 段が完了。**2-19（埋め込みメタデータの読み取り）は完了**——`ComicInfo.xml` / EPUB / PDF から著者・タイトル・シリーズ・巻数を読み、ファイル名由来の値より優先する。アプリとの結線が済み、実機のサンドボックス下で「登録フォルダを有効化（設定を確認・調整し、実ファイル名でのプレビューを見てから決められる）→ 実ファイルを DB へ取り込み → ラベル自動付与 → 設定を編集して再スキャンで反映 → JSON へ書き出し → ライブラリを削除 → 取り込みで復元」まで動作を確認済み。**
