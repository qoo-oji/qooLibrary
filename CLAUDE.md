...
**何も選んでいないのに「保管庫から戻す」と出ていた。** 押せない（無効）が、
何も選んでいない画面の一番目立つ位置に、**まれで逆向きの操作**が主目的として
座ることになる。「選択が空なら移す側」に変えた（回帰テスト追加）。

##### 実機で確認できなかったもの

**改名・検索・ラベルの新規作成**——いずれも**テキスト入力**を要するが、
合成入力（unicode も実キーコードも）がこのウインドウの SwiftUI `TextField` に
届かない。**同じウインドウの既存フィールド（設定ウインドウ由来のグループ名欄）
でも同じ**なので、実装ではなく合成操作側の限界と判断した（教訓 35 の再確認）。
論理は自動テストと変異検証で固定済み。
...
| # | 内容 |
|---|---|
| 1 | **実機検証で確認できなかったのは、テキスト入力を要するもの**（改名の衝突からの統合誘導・検索欄・ラベルの新規作成）だけ。合成入力が SwiftUI の `TextField` に届かない既知の制約で、**手動確認を依頼したい**。他は上表のとおり確認済み |
...
55. **`Window(id:)` のウインドウは ⌘W で閉じても `@State` が生き残る。**
